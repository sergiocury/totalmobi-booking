...
describe("filtrarPorPreferencia", () => {
  ...
  it("as fronteiras pertencem ao período que começa", () => {
    ...
    expect(
      horas(filtrarPorPreferencia(DIA, { periodo: "manha" })),
    ).not.toContain("12:00");
    expect(horas(filtrarPorPreferencia(DIA, { periodo: "tarde" }))).toContain(
      "12:00",
    );
    expect(
      horas(filtrarPorPreferencia(DIA, { periodo: "tarde" })),
    ).not.toContain("18:00");
    expect(horas(filtrarPorPreferencia(DIA, { periodo: "noite" }))).toContain(
      "18:00",
    );
  });
});
...
describe("descreverPreferencia", () => {
  it("sem pedido não há nada a explicar", () => {
    ...
    expect(
      descreverPreferencia({ periodo: null, horaMinima: null }),
    ).toBeNull();
  });

  it("nomeia cada limite", () => {
    expect(descreverPreferencia({ periodo: "tarde" })).toBe("de tarde");
    expect(descreverPreferencia({ horaMinima: "15:00" })).toBe(
      "depois das 15:00",
    );
    expect(descreverPreferencia({ horaMaxima: "12:00" })).toBe(
      "antes das 12:00",
    );
  });
  ...
  it("junta mínimo e máximo num intervalo", () => {
    expect(
      descreverPreferencia({ horaMinima: "10:00", horaMaxima: "12:00" }),
    ).toBe("entre as 10:00 e as 12:00");
  });

  it("hora exata diz-se como hora, não como intervalo", () => {
    expect(
      descreverPreferencia({ horaMinima: "15:00", horaMaxima: "15:00" }),
    ).toBe("às 15:00");
  });

  it("junta período e limite", () => {
    expect(
      descreverPreferencia({ periodo: "tarde", horaMinima: "16:00" }),
    ).toBe("de tarde depois das 16:00");
  });
  ...
  it("período desconhecido não apaga o limite de hora", () => {
    expect(
      descreverPreferencia({ periodo: "madrugada", horaMinima: "15:00" }),
    ).toBe("depois das 15:00");
  });
});
