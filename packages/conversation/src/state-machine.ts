...
function slotEscolhido(
  ...
  if (numero) {
    ...
    if (indice >= 0 && indice < oferecidos.length)
      return oferecidos[indice]!.iso;
  }
...
export function proximoTurno(entrada: EntradaDoTurno): Resposta {
  ...
  if (estado === "SELECTING_SLOT") {
    ...
    if (escolhido) {
      ...
      const hora =
        contexto.slotsOferecidos?.find((s) => s.iso === escolhido)?.hora ?? "";
  ...
  return {
    ...
    contexto,
    texto:
      "Desculpe, não percebi. Quer marcar, alterar ou cancelar uma marcação?",
