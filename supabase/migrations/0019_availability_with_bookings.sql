-- =============================================================================
-- 0019 — O dataset passa a saber o que já está marcado
-- =============================================================================
--
-- Fecha o buraco que ficou aberto de propósito na 0013: até agora o dataset
-- devolvia `busy: []` porque a tabela `bookings` ainda não existia. Existe
-- desde a 0015.
--
-- O QUE SAI, E O QUE NÃO SAI
--
-- Sai o `blocked_range` — buffers incluídos, exatamente o intervalo que a
-- constraint de exclusão compara. Se saísse só `start_at`/`end_at`, o motor
-- ofereceria horas que a base de dados depois recusava.
--
-- Não sai mais nada. Nem quem marcou, nem o serviço, nem o telefone. Uma
-- página pública que diz "10:00 ocupado" está correta; uma que deixasse
-- deduzir *quem* lá está seria uma fuga — e num consultório médico, uma fuga
-- de dados de saúde.
--
-- Esta função é `SECURITY DEFINER` e lê `bookings`, que o `anon` não pode ler
-- pela API. É precisamente por isso que ela existe: dá a informação mínima
-- necessária, sem abrir a tabela.
-- =============================================================================
    ...
    'staff', coalesce((
      ...
      from (
        select jsonb_build_object(
          ...
          -- O que já está ocupado. Sai o `blocked_range` **inteiro**, buffers
          -- incluídos — é o mesmo intervalo que a constraint de exclusão usa.
          -- Devolver só `start_at`/`end_at` faria o motor oferecer horas que a
          -- base de dados depois recusava, e o cliente via o erro só depois de
          -- ter escolhido.
          --
          -- Nada mais sai daqui: nem quem marcou, nem o quê, nem o telefone.
          -- Para o motor, uma marcação é um intervalo ocupado e mais nada.
          'busy', coalesce((
            select jsonb_agg(jsonb_build_object(
                     'startsAt', lower(b.blocked_range),
                     'endsAt',   upper(b.blocked_range)
                   ) order by b.start_at)
            from booking.bookings b
            where b.staff_id = st.id
              and b.occupies_slot
              and b.blocked_range && tstzrange(
                    p_from::timestamptz, (p_to + 1)::timestamptz, '[)')
          ), '[]'::jsonb)
        ) as pessoa
...
comment on function booking.availability_dataset(uuid, uuid, date, date, uuid) is
  'Tudo o que o motor precisa, numa chamada: horários, exceções, ausências e os intervalos já ocupados. Só colunas públicas.';
